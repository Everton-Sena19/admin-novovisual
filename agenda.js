// agenda.js
import {gruposPodemCoexistir} from "./utils.js";
import {
...
async function fetchAppointmentsDayForColecao(dateYmd, colecao) {
  ...
  snap.forEach((d) => {
    ...
    const telefone =
      v.clienteTelefone || v.telefoneCliente || v.phone || v.telefone || "";
...
async function fetchAppointmentsDay(dateYmd) {
  ...
  if (colecaoSel === "todos") {
    ...
    const tasks = profs.map((p) =>
      fetchAppointmentsDayForColecao(dateYmd, p.colecao)
    );
